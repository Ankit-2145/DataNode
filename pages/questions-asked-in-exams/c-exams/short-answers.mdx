...
An **operand** is a quantity on which an operator performs an action in an expression. In the expression `a + b`, both `a` and `b` are operands, and `+` is the operator that defines the addition operation.
...
### (vi) What are the characteristics of C language?
...
### (vii) What are Identifiers?

Identifiers in C are names given to variables, functions, arrays, or user-defined types. They must begin with a letter or an underscore, followed by alphanumeric characters. Identifiers are case-sensitive, and they must not match C keywords like `int`, `if`, or `return`.
...
### (viii) Explain Break Statement.

A `break` statement in C is used to exit a loop (`for`, `while`, `do-while`) or a `switch` block prematurely. When the `break` statement is executed, control passes to the first statement following the loop or switch block, stopping further iteration.
...
### (ix) Differentiate between formal argument and actual argument.
...
### (x) What are Escape Sequences?

Escape sequences in C are special character combinations starting with a backslash (`\`), used to represent non-printable or special characters in a string or character. Common escape sequences include:
- `\n` : Newline
- `\t` : Tab
- `\\` : Backslash
- `\'` : Single Quote
