...
export default function featuresGird() {
  return (
    <section className="p-5 mt-0">
      ...
      <BentoGrid className="max-w-6xl mt-14 mx-auto md:auto-rows-[20rem]">
        {items.map((item, i) => (
          <BentoGridItem
            key={i}
            title={item.title}
...
const SkeletonFive = () => {
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-xs text-neutral-500">We got your back</p>
        <div className="h-6 w-6 rounded-full bg-gradient-to-r from-pink-500 to-violet-500 flex-shrink-0" />
      </motion.div>
    </motion.div>
  );
...
const items = [
  ...
  {
    title: "Tech Trends and Industry Updates",
    description: (
      <span className="text-sm">
        Information on the latest technology trends across different domains,
        helping students stay updated with the current industry landscape.
      </span>
    ),
    header: <SkeletonFour />,
